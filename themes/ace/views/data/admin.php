<?php
$this->pageTitle = Yii::t ( 'vcos', '管理员列表' );
$this->pageTag = 'index';
...
if (in_array ( '115', $auth ) || $auth [0] == '0') {
	$canadd = TRUE;
} else {
	$canadd = False;
}
if (in_array ( '116', $auth ) || $auth [0] == '0') {
	$canedit = TRUE;
} else {
	$canedit = False;
}
if (in_array ( '117', $auth ) || $auth [0] == '0') {
	$candelete = TRUE;
} else {
	$candelete = FALSE;
}
...
<div class="main-container" id="main-container">
	...
	<div class="main-container-inner">
							...
							<div class="col-xs-12">
								<div class="table-responsive">
										...
										<table id="sample-table-1"
											...
											<thead>
												<tr>
													...
													<th><?php echo yii::t('vcos', '管理员账号')?></th>
													<th><?php echo yii::t('vcos', '管理员昵称')?></th>
													<th><?php echo yii::t('vcos', '管理员邮箱')?></th>
													<th><?php echo yii::t('vcos', '所属权限分组')?></th>
													<th><?php echo yii::t('vcos', '最后登录ip')?></th>
													<th><?php echo yii::t('vcos', '最后登录时间')?></th>
											...
											<tbody>
													...
													<td><?php echo $row['role_name']?$row['role_name']:'无权限';?></td>
													<td><?php echo $row['last_login_ip'];?></td>
													<td><?php echo $row['last_login_time'];?></td>
													<td><?php echo $row['admin_state']? yii::t('vcos', '启用'): yii::t('vcos', '禁用');?></td>
													<td>
														<div
															class="visible-md visible-lg hidden-sm hidden-xs btn-group">
                                                                    <?php if($canedit){?>
                                                                    <a
																href="#" id="<?php echo $row['admin_id'];?>"
																class="edit btn btn-xs btn-info"
																title="<?php echo yii::t('vcos', '编辑');?>"> <i
																class="icon-edit bigger-120"></i>
															</a>
                                                                    <?php } ?>
                                                                    <?php if($candelete){?>
                                                                    <a
																href="#" class="btn btn-xs btn-warning delete"
																id="<?php echo $row['admin_id'];?>"
																title="<?php echo yii::t('vcos', '删除');?>"> <i
															...
															<div class="inline position-relative">
																...
																<ul
																	class="dropdown-menu dropdown-only-icon dropdown-yellow pull-right dropdown-caret dropdown-close">
                                                                            <?php if($canedit){?>
                                                                            <li>
																		<a href="#" id="<?php echo $row['admin_id'];?>"
																		class="edit tooltip-info" data-rel="tooltip"
																		title="<?php echo yii::t('vcos', '编辑');?>"> <span
                                                                            ...
                                                                            <?php if($candelete){?>
                                                                            <li>
																		<a href="#" class="tooltip-info delete"
																		id="<?php echo $row['admin_id'];?>" data-rel="tooltip"
																		title="<?php echo yii::t('vcos', '删除');?>"> <span
									...
									<div class="center">
                                                    <?php if($canadd){?>
                                                    <a
											href="<?php echo Yii::app()->createUrl('Auth/admin_add');?>"
											class="btn btn-xs"> <i
											class="icon-pencil align-top bigger-125"></i> <span
											class="bigger-110 no-text-shadow"><?php echo yii::t('vcos', yii::t('vcos', '添加'))?></span>
										</a>
                                                    <?php } ?>
                                                </div>
									<div class="center">
		...
		<!-- #dialog-confirm -->
		<form action="<?php echo Yii::app()->createUrl("Auth/admin_edit");?>"
			method="post">
...
<script type="text/javascript">
    jQuery(function($) {
        ...
        $( ".delete" ).on('click', function(e) {
            var $a = $(this).attr("id");
            e.preventDefault();
            $.post("checkismyself",{id:$a},function(data){
                if(data > 0){
                    $('#danger').hide();
                    $('.widget-header h4').html("<i class='icon-warning-sign red'></i><?php echo yii::t('vcos', '不能删除！')?>");
                    $('#dialog-confirm-content').html("<?php echo yii::t('vcos', '你不能把自己删除！')?>");
                    $('#confirm').hide();
                }
            });
                ...
                buttons: [
                    {
                        ...
                        "id" : "danger",
                        click: function() {
                            location.href="<?php echo Yii::app()->createUrl("Auth/admin");?>"+"?id="+$a;
                            $( this ).dialog( "close" );
                        }
                    }
                    ,
