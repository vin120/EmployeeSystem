...
class AssessmentController extends Controller
{
	public function actionAssessment_attendance(){
		...
		/* 	if ($_POST){
			...
			if ($_POST){
			$iswhere=array(0=>"employee_code",1=>"type",2=>"leaveorholiday_id",3=>"date_of_start",4=>"date_of_end",5=>"count_day",6=>"remark");
			$selectdata=new seletedata();
			$data=$selectdata->inputdata($iswhere);
		
			if ($data==''){
				$this->render("attendance",array('t'=>$t,'page'=>1,'count'=>1,'depinfo'=>array(),'leaveinfo'=>array(),'postinfo'=>array()));
			}
			else{
				try{
					$transaction=$db->beginTransaction();
					foreach ($data as $k=>$v){//判断输入的exal格式是否按要求
						$g=$k+1;
						$id=employment_profiles::model()->find("employee_code=:employee_code", [':employee_code' =>$v['employee_code']]);
						if (empty($id)){
	...
	if ($del_id!=''){
		...
		try{
			$leave_log= $db->createCommand()->delete('vcos_leave_log'
					, "id=:id", [
					':id' =>$data[0]
					]);
		...
		elseif ($data[1]=='holiday'){//holiday提交过来的数据进行删除
			try{
				$holiday_log= $db->createCommand()->delete('vcos_holiday_log'
						, "id=:id", [
						':id' =>$data[0]
						]);
	...
	public function actionEditattendanceUpdate(){
		...
		$transaction=$db->beginTransaction();
		if ($del_id!=''){
			$employee_code=holiday_log::model()->find("id=:id", [':id' =>$del_id]);
	...
	public function actionAssessment_month(){
		...
		elseif ($t==2){//第三个div
			...
			$viewName= $this->getAction()->getId();
			if ($_POST){
			$iswhere=array(0=>"employee_code",1=>"month_of_assessment",2=>"score",3=>"bonus",4=>"remark");
			$selectdata=new seletedata();
			$data=$selectdata->inputdata($iswhere);
			...
			$this->render("month",array('t'=>$t,'page'=>1,'count'=>1,'depinfo'=>array(),'leaveinfo'=>array(),'postinfo'=>array(),'month_assessmentinfo'=>array()));
	...
	public function actionMonthUpdate(){
		...
		if ($del_id!=''){
			try{
				$month_assessment= $db->createCommand()->delete('vcos_month_assessment'
						, "id=:id", [
						':id' =>$del_id
						]);
	...
	public function actionAssessment_quarter(){
		...
		elseif ($t==2){//第三个div
			$viewName= $this->getAction()->getId();
			if ($_POST){
			$iswhere=array(0=>"employee_code",1=>"season_of_assessment",2=>"score",3=>"bonus",4=>"remark");
			$selectdata=new seletedata();
			$data=$selectdata->inputdata($iswhere);
			...
			$this->render("quarter",array('t'=>$t,'page'=>1,'count'=>1,'depinfo'=>array(),'leaveinfo'=>array(),'postinfo'=>array(),'season_assessmentinfo'=>array()));
	...
	public function actionSeasonUpdate(){
		...
		if ($del_id!=''){
			try{
				$season_assessment= $db->createCommand()->delete('vcos_season_assessment'
						, "id=:id", [
						':id' =>$del_id
						]);
