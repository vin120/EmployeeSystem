...
class InformationController extends Controller
{
	...
	public function actionProfileUpdate(){
		...
		$transaction=$db->beginTransaction();
		if($edit_id!=''){//如果按编辑
			$employee_id=employee::model()->find( "employee_code=:employee_code", [':employee_code' =>$edit_id]);
			$employee_id=$employee_id['employee_id'];
			$this->redirect("Information_addfile?employee_id=$employee_id");
		}
		elseif ($del_id!=''){//如果按删除
			try {	
				$employment_profiles= $db->createCommand()->delete('vcos_employment_profiles'
						, "employee_code=:employee_code", [
						':employee_code' =>$del_id
				]);
				$employee= $db->createCommand()->delete('vcos_employee',
						"employee_code=:employee_code", [
						':employee_code' =>$del_id
				]);
				$transaction->commit();
...
/* 		更新	 */	
	...
	public function actionInformation_certificate(){
		...
		$this->render('certificate',array('posts'=>$posts,'count'=>$count,'name'=>$_name,'code'=>$_code,'verification'=>$_verification,'page'=>$page,'isPage'=>$_isPage,'country'=>$country,'depinfo'=>$depinfo,
				'postinfo'=>$postinfo,
...
public function actionInformation_title(){
	...
	$this->render('title',array('posts'=>$posts,'count'=>$count,'verification'=>$_verification,'page'=>$page,'isPage'=>$_isPage,'depinfo'=>$depinfo,
			'postinfo'=>$postinfo,
			'certificate'=>$certificate,
			'income'=>$posts,
			'employee_code'=>$employee_code,
...
public function actionInformation_contract(){
	...
	$this->render('contract',array('posts'=>$posts,'count'=>$count,'name'=>$_name,'code'=>$_code,'verification'=>$_verification,'page'=>$page,'isPage'=>$_isPage,'country'=>$country,'depinfo'=>$depinfo,
			'postinfo'=>$postinfo,
			'certificate'=>$certificate,
			'income'=>$posts,
			'employee_code'=>$employee_code,
...
public function actionInformation_train(){
	...
	$this->render('train',array('posts'=>$posts,'count'=>$count,'verification'=>$_verification,'page'=>$page,'isPage'=>$_isPage,'depinfo'=>$depinfo,
			'postinfo'=>$postinfo,
			'certificate'=>$certificate,
			'income'=>$posts,
			'employee_code'=>$employee_code,
...
public function actionTrainUpdate(){
    ...
    if ($del_id!=''){
    	try{
    	$train_management= $db->createCommand()->delete('vcos_train_management'
    			, "id=:id", [
    			':id' =>$del_id
    			]);
    	$transaction->commit();
...
public function actionAddDelete(){
	...
	if ($del_code!=''){
		...
		try{
			...
			$certificate_management= $db->createCommand()->delete('vcos_certificate_management'
					, "id=:id", [
					':id' =>$del_id[0]
					]);
			...
			elseif ($del_id[1]=='title'){
				$title_management= $db->createCommand()->delete('vcos_title_management'
						, "id=:id", [
						':id' =>$del_id[0]
						]);
			...
			elseif ($del_id[1]=='contract'){
				$contract_management= $db->createCommand()->delete('vcos_contract_management'
						, "id=:id", [
						':id' =>$del_id[0]
						]);
			...
			elseif ($del_id[1]=='qualification'){
				$contract_management= $db->createCommand()->delete('vcos_qualification_management'
						, "id=:id", [
						':id' =>$del_id[0]
						]);
...
public function actionAddsave(){
	...
	try {
		...
		if (empty($_POST['employee_id'])){
			...
			$res=$db->createCommand()->update("vcos_employee",$data,
					"employee_code=:employee_code", [  
       				 ':employee_code' =>$_POST['employee_code']
            ]);
			$res=$db->createCommand()->update("vcos_employment_profiles",$data_p,
					"employee_code=:employee_code", [
					':employee_code' =>$_POST['employee_code']
					]);
...
public function actionAttachment(){
	...
	$transaction=$db->beginTransaction();
	try{
	$res=$db->createCommand()->delete("vcos_file_path",
			"id=:id", [
			':id' =>$id
			]);
	$transaction->commit();
...
if( $model->save() ){  
    ...
    $this->render('aa',array('model'=>$model));  
} 
}
